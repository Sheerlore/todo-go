...
func DeleteItem(w http.ResponseWriter, r *http.Request) {
	...
	if err == false {
		w.Header().Set("Content-Type", "application/json")
		io.WriteString(w, `{"deleted": false, "error": "Record Not Founc"}`)
	} else {
...
func GetItemByID(Id int) bool {
	...
	if result.Error != nil {
		log.Warn("TodoItem not in database")
		return false
	}
	return true
}
...
func GetIncompletedItems(w http.ResponseWriter, r *http.Request) {
	log.Info("Get Imcompleted TodoItems")
...
func main() {
	...
	handler := cors.New(cors.Options{
		AllowedMethods: []string{"GET", "POST", "DELETED", "PATCH", "OPTIONS"},
	}).Handler(router)
